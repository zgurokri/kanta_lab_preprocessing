...
def lab_unit_regex(df,args,map_mask=None):
    ...
    # LOG CHANGES
    unit_df = df[['FID', 'EVENT_DATETIME','TEST_NAME_ABBREVIATION','MEASUREMENT_UNIT']].copy()
    unit_df['OLD'] = old_col
    unit_df['SOURCE'] = "regex"    
    unit_mask = (unit_df["OLD"] != unit_df[col])
    unit_df[unit_mask][['FID', 'EVENT_DATETIME','TEST_NAME_ABBREVIATION','OLD','MEASUREMENT_UNIT','SOURCE']].to_csv(args.unit_file, mode='a', index=False, header=False,sep="\t")
    return df
